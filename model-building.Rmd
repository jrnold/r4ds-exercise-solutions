...
A one log unit change in `carat` is associated with a `a_1` unit change in `price`.
But what does that mean on the linear scale?

$$
\begin{aligned}
\Delta \log_b (\mathtt{price}) & = a_0 + a_1 \log_b (k \times  \mathtt{carat}) - a_0 + a_1 \log_b (\mathtt{carat})  \\
&= a_1 \left( \log_b(\mathtt{carat}) - \log_b (k\times  \mathtt{carat}) \right) \\
&= a_1 \log_b \left( \frac{ k \times \mathtt{carat} }{\mathtt{carat}} \right) \\
&= a_1 \log_b k
\end{aligned}
$$

A multiplicative increase of $k$ in `carat` is associated with a $a_1 \log_b k$ 
multiplicative increase in `price`. 

For small changes in the covariate, there are some useful approximations.
$$
\frac{d}{dx} \log(y) =  \frac{d}{dx} \left( a_0 + a_1 \log(x) \right)  \\
= a_1 \frac{dx}{x}
$$
$$
\frac{d y}{dx} =  \frac{d}{dx} e^{a_0} x^{a_1} = e^{a_0} a_1 x^{a_1 - 1} dx = (e^{a_0} x^{a_1}) (dx / x) = \frac{y dx}{x}
$$

Some useful approximations that come from Taylor expansions:
$$
\begin{aligned}[t]
\log (x) &\approx 1 - x && \text{when } x \approx 1 \\
\log (x + 1) &\approx x && \text{when } x \approx 0
\end{aligned}
$$


$$
\Delta \log y = \alpha_1 (\log(x + \Delta x) - \log(x)) \\
= \alpha_1 \log \left( \frac{x + \Delta x}{x} \right) \\
= \alpha_1 \log \left( \frac{\Delta x}{x} + 1 \right) \\
\approx \alpha_1 \Delta x / x = \%\Delta \\
$$
...
`r round(lresid2_summary$p975, 2)`, which correspond to  `r round(100 * (1 - (2 ^ lresid2_summary$p025)))`--`r round(100 * (1 - 2 ^ lresid2_summary$p975_err))`.
...
I have no idea how to price diamonds, so this would be useful to me in 
order to understand a reasonable price range for a diamond. If I were buying
and sellind diamonds as a business, I may require a better model.
...
Looking at only day of the week, we see that Sunday flights are on average longer than the rest of the day of the week flights, but not as long as Saturday flights.
Perhaps Saturday flights are vacation flights.
```{r}
flights %>%
  mutate(date = make_date(year, month, day),
