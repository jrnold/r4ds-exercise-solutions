...
1.  What word has the higher number of vowels? What word has the highest proportion of vowels? (Hint: what is the denomination?)
...
What word has the higher number of vowels?
```{r}
df <- tibble (
  word = words,
  i = seq_along(word)
...
df %>%
  mutate(
    prop = str_count(word, "[aeiou]") / str_length(word)
  )%>%
  arrange(desc(prop))
```
