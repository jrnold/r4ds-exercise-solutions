...
1.  Words that end with "-ed" but not ending in "-eed". This handles the special case of "-ed", as well as words with a length great than two.
    ...
    ```{r }
    str_subset(stringr::words, "^ed$|[^e]ed$")
    ```
...
In the full English language, [no](https://en.wiktionary.org/wiki/Appendix:English_words_containing_Q_not_followed_by_U) 
However, the examples are few, and mostly loanwords, such as "burqa" and "cinq".
Also, "qwerty".
It says something that I had to all of those examples to the list of words that spellchecking should ignore.
