...
However, the definition of continuous vs. categorical does not   
...
The typology of [levels of measurement](https://en.wikipedia.org/wiki/Level_of_measurement) is one such typology of data typs.

In this case the R data types largely encode the semantics of the variables; e.g. integer varaibles are stored as integers, categorical variables with no order are stored as character vectors and so on.
However, that is not always the case.
Instead, the data could have stored the cagtegorical `class` variable as an integer with values 1--7, where the documentation would note that 1 = "compact",  2 = "midsize", and so on.[^factor]
...
Varibles with an ordering support sorting and calculating quantiles.
...
However, this is not always the case, with `geom_bar()` and `stat_count()` and `geom_histogram()` and `stat_bin()` as notable counter-examples.
...
Combining `geom_count()` with jitter, which is specifified with the `position` argument to `geom_count()` rather than its own geom, helps overplotting a little.
```{r}
ggplot(data = mpg, mapping = aes(x = cty, y = hwy, color = class)) +
