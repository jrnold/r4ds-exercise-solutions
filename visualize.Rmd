...
`geom_jitter()` adds random noise to the points of the graph. This method slightly reduces the accuracy of the points, but makes the plot more readable by reducing overplotting.
...
`geom_count()` faithfully plots points at their correct locations, however `geom_count()` is less readable than `geom_jitter()` when adding a third variable as color aesthetic. Compare the following two plots which add a `color = class` aesthetic.
