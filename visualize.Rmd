...
The `gggplot()` function creates the background of the plot, 
but since nothing else was specified, nothing else is drawn.
...
In this data, `mpg` takes `r n_drv` values and `class` takes `r n_class` values,
meaning that there are only `r n_comb` values that could be plotted on a scatterplot of `mpg` vs. `class`.
In this data, there `r n_comb_actual` values of (`mpg`, `class`) are observed.
...
The following code uses functions introduced in a later section, but may be of interest. 
...
In the previous plot, the are many missing tiles.
This missing tiles represent unobserved combinations of`class` and `drv` values.
...
This also explains why, in exercise 1, the expression `colour = "blue"` created a categorical variable with only one category: "blue".
...
In the former, as the number of points increases, there is likely to be more
overlap.
...
The default message says that `stat_summary()` uses the `mean` and `sd` to calculate the point, and range of the line. So let's use the previous values of `fun.ymin`, `fun.ymax`, and `fun.y`:
```{r}
ggplot(data = diamonds) +
  geom_pointrange(
    mapping = aes(x = cut, y = depth),
...
With jittering, the plot more clearly shows the relationship between `cty` and `hwy`.
...
However, we can adjust them. Here are few a examples to understand how adjusting
...
When `height = 0`, there is no vertical or horizontal jitter.
...
If `coord_polar()` is specified without `theta = "y"`, then the resulting plot is called a bull’s-eye chart.
