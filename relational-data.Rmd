...
Drawing the approximate flight paths requires the latitude and longitude of the both 
the origin and desination airports of each flights.
This would require the `flights` and `airports` tables.
...
This plots the approximate flightpaths of the first 100 flights in the `flights` dataset.
```{r}
flights_latlon %>%
  slice(1:100) %>%
...
For example, the diagrams in *R for Data Science* were created with [OmniGraffle](https://www.gliffy.com/), and their sources can be found in its [GitHub repository](https://github.com/hadley/r4ds/tree/master/diagrams).
...
In the previous diagrams, I do not consider `teamID` and `lgID` as foreign keys even though they appear in multiple tables (and have the same meaning) because they are not primary keys for any of the tables considered in this exercise.
The `teamID` variable references `Teams$teamID`, and `lgID` does not have its own table.

The `Batting`, `Pitching`, and `Fielding` tables all have a primary key consisting of the `playerID`, `yearID`, and `stint` columns.
However, there are no foreign key relations between them. 
Logically the `playerID`, `yearID`, and `stint` columns refer to the same concepts.
When a (`playerID`, `yearID`, `stint`) tuple exists in two tables, it will be a 1-1 relationship.

-  `Batting`: (`playerID`, `yearID`, `stint`)
-  `Fielding`: (`playerID`, `yearID`, `stint`, `POS`).
   There are separate rows for a player who played different positions within a stint.
...
Since the data contains all flights from New York City airpots, this is the list of
