...
Surprisingly not. If anything, departure delay decreases slightly with the age of the plane.
...
Plot departure delay by age.
```{r}
ggplot(plane_cohorts, aes(x = age, y = dep_delay)) +
  geom_point() +
  geom_line()
```

Plot arrival delay by age.
```{r}
ggplot(plane_cohorts, aes(x = age, y = arr_delay)) +
  geom_point() +
  geom_line()
```
...
-   What about arrival delays? 

-   What would cause departure and arrival delays? 
    Which of these would you expect to be more related to a plane's age.
