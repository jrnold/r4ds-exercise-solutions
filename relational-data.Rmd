...
**Ed. I'm unsure about the answer to this question.**

To find the 48 hours with the worst delays, I will group by `year`, `month`,
and `day`, sum arrival delays for all flights, and then add the arrival delays
from the previous day to it.
```{r}
delays_48 <- flights %>%
  group_by(year, month, day) %>%
  summarise(dep_delay = mean(dep_delay, na.rm = TRUE)) %>%
  arrange(year, month, day) %>%
  mutate(dep_delay_48 = 0.5 * (dep_delay + dep_delay)) %>%
  filter(!is.na(dep_delay_48))
delays_48 
```

The question does not define exactly what is meant by "worst delays". It could
be arrival delays or departure delays. I could ignore early departures or arrivals
of include them. It could sum all flights for the total delay, or consider the 
average delay. There are many reasonable ways to define "worst delays". I chose
the sum of arrival delays.

```{r}
delays_48 %>%
  arrange(desc(dep_delay_48)) %>%
  select(year, month, day, dep_delay_48) %>%
  head()
```

I need to create a dataset which summarizes weather data for each 48 hours in order 
to cross-reference it with the delay data. The primary key in the `weather` data 
is (`year`, `month`, `day`, `hour`, `origin`). The desired output will have 
(`year`, `month`, `day`) as a primary key. I will first calculate daily summaries
for each `origin` of temperature, wind speed, and precipitation (the variable in `weather` that I
think are most likely to be associated with delays). Then I will use the average
of of `origin` daily values as the overall daily value of the weather.
This is not the only reasonable way to generate daily weather summaries, but the
final daily dataset should have (`year`, `month`, `day`) as its primary key.
```{r}
weather_daily <- weather %>%
  ...
  summarise(temp_max = max(temp), 
            ...
            precip = sum(precip)) %>%
  # average over all airports for daily values
  group_by(year, month, day) %>%
  summarise(temp_max = mean(temp_max),
            temp_min = mean(temp_min),
            wind_speed_mean = mean(wind_speed_mean),
            wind_speed_max = mean(wind_speed_max),
            precip = mean(precip)) %>%
  ungroup()
```
From `weather_daily`, I will create 48-hour summaries of the weather variables.
```{r}
weather_48 <- weather_daily %>%
  arrange(year, month, day) %>%
  mutate(temp_max = pmax(temp_max, lag(temp_max)),
         temp_min = pmin(temp_min, lag(temp_min)),
         wind_speed_mean = 0.5 * (wind_speed_mean + lag(wind_speed_mean)),
         wind_speed_max = pmax(wind_speed_max, lag(wind_speed_max)),
         precip = precip + lag(precip))

```
Merge the 48-hour delay data with the 48-hour weather data.
```{r}
delays_48 <- left_join(delays_48, weather_48, by = c("year", "month", "day"))
```

However, I don't see much relationship between weather and 48-hour delays.
```{r}
ggplot(delays_48, aes(x = precip, y = wind_speed_mean, size = dep_delay_48,
                      color = temp_max)) +
  geom_point()
...
```{r}
ggplot(delays_48, aes(x = wind_speed_mean, y = dep_delay_48)) +
  geom_point()

```

```{r}
ggplot(filter(delays_48, wind_speed_max < 100),
       aes(x = wind_speed_max, y = dep_delay_48)) +
  geom_point()

```

```{r}
ggplot(delays_48, aes(x = temp_min, y = dep_delay_48)) +
  geom_point()

```

```{r}
ggplot(delays_48, aes(x = temp_max, y = dep_delay_48)) +
  geom_point()
