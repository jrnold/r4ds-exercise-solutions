...
1.  length is less than width, otherwise the length would be called the width, and
1.  depth is usually less than length or width.

```{r}
summarise(diamonds, mean(x > y), mean(x > z), mean(y > z))
```

It appears that depth (`z`) is always smaller than length (`x`) or width (`y`),
perhaps because a shallower depth helps when setting diamonds in jewelry.
Length is more than width in less than half the observations, the opposite of expectations.
