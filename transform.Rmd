...
```
```{r include=FALSE}
flights <- mutate(flights, -any_of(vars))
```

However,  `vars` is not a column xin `flights`, as is the case, then `select` will use the value the value of the , and select those columns.
