...
The relationship between the delay of a flight and the previous flight is almost exactly linear.
```{r message=FALSE}
ggplot(lagged_delays, aes(x = dep_delay, y = dep_delay)) +
  geom_point() +
  geom_smooth() +
  labs(x = "Departure Delay", y = "Previous Delay")
```

We can summarize this relationship by the average difference in delays:
```{r}
lagged_delays %>%
  summarise(delay_diff = mean(dep_delay - lag(dep_delay), na.rm = TRUE))
```
...
flights_with_zscore <- flights %>%
  ...
  mutate(air_time_mean = mean(air_time),
         ...
         n = n()) %>%
  ungroup() %>%
  mutate(z_score = (air_time - mean_air_time) / sd(air_time))
```
