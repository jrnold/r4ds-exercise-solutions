...
First, let's convert the times to a form more amenable to arithmetic operations using the same calculations as in the previous exercise.
```{r}
flights_airtime <-
  mutate(flights,
...
Was our expectation correct? Does `air_time = arr_time - dep_time`?
...
1.  The flight passes midnight, so `arr_time_minutes < dep_time_minutes`. 
...
However, plotting the data shows that is not the case.
```{r}
...
However, there are spikes at 0, 60 (1 hour), 120 (2 hours), and 180 (3 hours).
