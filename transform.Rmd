...
The **dplyr** package provides multiple functions for ranking, which differ in how they handle missing values: `row_number()`, `min_rank()`, `dense_rank()`.
The `row_number()` function is confusingly named.
It can create ranks for any column.
To see how they work, let's create a data frame with duplicate values in a vector and see how ranking handles ties.
```{r}
rankme <- tibble(
  x = c(10, 5, 1, 5, 5)
)
...
It is roughly equivalent to the index number of an element after sorting it.

The`min_rank()` and `dense_rank()` assign tied values the same rank, but differ in how they assign values to the next rank.
Both of these function assign ties the minimum index value.
For each set of values, the `min_rank()` function assigns a number equal to a number of elements less than that value plus one.
The `dense_rank()` assigns each set of values a number equal to the number of distinct *values* less than that value plus one.
Thus, `dense_rank()` has no gaps in its ranks.
...
I will use `min_rank()` since its results correspond to the common usage rankings, for example sports or other competitions.
```{r}
flights_delayed <- mutate(flights, dep_delay_rank = min_rank(desc(dep_delay)))
flights_delayed <- filter(flights_delayed, dep_delay_rank <= 10)
flights_delayed <- arrange(flights_delayed, dep_delay_rank)
select(flights_delayed, month, day, carrier, flight, dep_delay, dep_delay_rank)
```
Since there are no ties in 10 flights in this example, all three ranking functions will produce the same answer.
...
All trigonometric functions are all described in a single page, [Trig](`r rdoc_url("Trig", "base")`)
help page.
...
The function `sinpi(x)`, is equivalent to `sin(pi * x)`, and `sinpi()` and `tanpi()` are similarly defined.
```{r}
sinpi(x)
cospi(x)
tanpi(x)
...
Finally, R provides the function `atan2()`, which calculates the angle between the x-axis and the vector (0,0) to (`x`, `y`).
```{r}
...
This delay could be benificial, if less time is spent in the cramped confines of the airplane itself, or a negative, if that delayed time is still spent in the cramped confines of the airplane but unmoving on the runway.
...
If we interpret the question as what day of the year has the highest probability to have flight cancelled, here is the explore:
...
ggplot(cancelled_delayed) +
  geom_point(aes(x = date, y = prop_cancelled))

filter(cancelled_delayed, prop_cancelled > 0.18)
```
...
```
The fastest flight was `r fastest_flight$flightnum` from `r fastest_flight$origin` to
`r fastest_flight$dest` which departed on 
`r fastest_flight$date` at `r fastest_flight$time`.
It had an air time of `r round(fastest_flight$air_time)` minutes, compared to an average
flight time of `r round(fastest_flight$air_time_mean)` minutes for its route.
This was `r round(abs(fastest_flight$air_time_standard), 1)` standard deviations below
