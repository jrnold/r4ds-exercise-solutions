...
format_time <- function(x) {
  format(lubridate::make_datetime(hour = x %/% 100, min = x %% 100),
         "%H:%M")
}
fastest_flight <- standardized_flights %>%
  arrange(air_time_standard) %>%
  slice(1L) %>%
  mutate(date = format_ymd(year, month, day),
...
All of these answers have relied only on using a distribution of comparable observations to findn ununusal observations. 
In this case, the comparable obserservations were flights from the same origin to the same destination.
...
travelled at 770 mph due to a strong jet stream tailwind.
...
One explantion for unusually fast flights is that they are "making up time" in the
air. Commercial aircraft fly at a speed that is optimal for fuel consumption,
not one to maximize speed. If a flight is delayed on the ground, it may fly 
faster than usual in order to avoid being delayed on arrival. I would expect
that some of the unusually fast flights were delayed on departure.
...
amount a flight is delayed in air in two ways. The first is an absolute delay, `air_time - min(air_time)`. The second is a percentage increase in air time relative to the 
minimal air time, `(air_time - min(air_time)) / min(air_time) * 100`.
...
```
```{r include=FALSE}
most_delayed <- arrange(air_time_delayed,
                        desc(air_time_delay)) %>%
  mutate(date = format_ymd(year, month, day),
         time = format_time(dep_time),
...
The most delayed flights in air in minutes were the following.
```{r}
air_time_delayed %>%
...
The most delayed flight in terms of percentage of the fastest flight along that
