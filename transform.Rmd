...
That solved most discrepencies, but there are still some non matches because we
...
A common approach to finding unusual observations would be to calcualte the z-score of observations each flight.
```{r}
flights_by_zscore <- flights %>%
  filter(!is.na(air_time)) %>%
  group_by(dest, origin) %>%
  mutate(z_score = (air_time - mean(air_time)) / sd(air_time)) %>%
  select(dest, origin, year, month, day, 
         air_time,
         carrier, flight, z_score)
```

Possible unusual flights would be those with highly negative z-scores.
```{r}
print(head(arrange(flights_by_zscore, z_score), 25),
      n = 25)
```

A better approach is to compare the actual air time with the scheduled air time.
```{r}
mutate(flights,
       sched_sched_dep_time - sched_arr_time)
```


```{r}
ggplot(flights_by_zscore, aes(x))
```
