...
Count will group a dataset on the given variable and then determine the number of instances within each group. The easiest way to do this is to `group_by` the given variable and then find the length of each group.  To make it match count, the value should go in a new column `n`.
```{r}
not_cancelled %>% group_by(dest) %>% summarize(n = length(dest))
```

For a weighted count, just take the sum of the weight variable in each group.
```{r}
