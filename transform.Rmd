...
If the missing value were `TRUE`, then `TRUE & FALSE == TRUE`,
and if the missing value was `FALSE`, then `FALSE & FALSE == TRUE`.

```{r}
NA | FALSE
```

For `NA | FALSE`, the value is unknown since `TRUE | FALSE == TRUE`, but `FALSE | FALSE == TRUE`.
...
How the code is evaluated will depend on whether or not `vars` is a column in `flights`.
...
I have not debuged the cause of this bad data, so these rows are dropped for
