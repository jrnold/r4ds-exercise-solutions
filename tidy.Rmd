...
```{r}
preg_tidy <- gather(preg, sex, count, -pregnant)
preg_tidy
```
...
See the discussion about missing values in Section 12.5.
...
```{r}
preg_tidy2 <- gather(preg, sex, count, -pregnant, na.rm = TRUE)
preg_tidy2
```
...
preferrable to store them as logical vectors.
I will 
```{r}
