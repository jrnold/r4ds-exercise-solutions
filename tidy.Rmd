...
<div class="alert alert-error hints-alert">

This chapter is out of data with the current version of R4DS.
That chapter has been rewritten to use the functions `pivot_wider()` and `pivot_longer()` which were introduced in [tidyr 1.0.0](https://tidyr.tidyverse.org/news/index.html#pivoting).
The current answers use the older `pivot()` and `gather()` functions.
Additionally, the exercises themselves have changed.
...
To select the columns `1999` and `2000`, the numes must be surrounded in backticks (`\``) or provided as strings.
...
Spreading this data frame using `pivot_wider()` produces columns that are lists of numeric vectors because the `name` and `key` columns do not uniquely identify rows.
