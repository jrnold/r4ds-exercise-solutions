...
To tidy `preg`, we need to use `gather()`. 
...
This will be discussed in the upcoming  [Missing Values](http://r4ds.had.co.nz/tidy-data.html#missing-values-3) section.
Supposing that these data represent observations from a species in which it is impossible for males to get pregnant (meaning, not seahorses), then that missing value is structural. 
In the non-tidy data frame we had to include that structural missing value explicitly with an `NA` entry.
However, in the tidy version we can drop that row since it is an impossible  combination.
...
In this example, one of the entries, `"d,e"`, has too few elements.
