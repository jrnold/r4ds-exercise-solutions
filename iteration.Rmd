...
1.  Determine the type of each column in `nycflights13::flights`.
    ...
    For example, the class of `time_hour` is `r class(time_hour)`.
...
<div class="question">
Rewrite `map(mtcars, function(df) lm(mpg ~ wt, data = df))` to eliminate the anonymous function.
</div>

<div class="answer">

Note that there is a bug in the question code.
```{r}
map(mtcars, function(df) lm(mpg ~ wt, data = df))
```
The reasons for this bug are technical, but the following code will work.
```{r}
