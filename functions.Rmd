...
I can confirm this by testing a function that allows for `na.rm` as an argument`r EndQuestion()`
```{r}
...
This function calculates the proportion of `NA` values in a vector`r EndQuestion()`
```{r}
prop_na <- function(x) {
  mean(is.na(x))
}
...
The coefficient of variation is the standard deviation divided by the mean`r EndQuestion()`
```{r}
...
The lyrics of one of the [most common versions](https://en.wikipedia.org/wiki/Little_Bunny_Foo_Foo) of this song are`r EndQuestion()`
...
Here's one way of writing this`r EndQuestion()`
```r
...
The function `f2` drops the last element`r EndQuestion()`
```{r}
f2(1:3)
f2(1:2)
f2(1)
...
To have intervals open on the left (using `<`), I change the argument to `right = FALSE``r EndQuestion()`
```{r}
temp <- seq(-10, 50, by = 5)
